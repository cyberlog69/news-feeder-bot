...
function formatBriefingMarkdown(briefing) {
  ...
  briefing.strategicActionItems.forEach((action, i) => {
    ...
    if (idx > 0) {
      lines.push(`${i + 1}. **${action.slice(0, idx)}:** ${action.slice(idx + 1).trim()}`);
    } else {
 ...
 * All DB/feed-sourced values are HTML-escaped to prevent XSS.
 * @param {object} briefing
 * @returns {string}
 */
function formatBriefingHtml(briefing) {
  ...
  const kevRows = briefing.cisaKevWatchlist.map((k) => `
    <tr>
      <td style="font-weight:700;color:#dc2626">${esc(k.cveId)}</td>
      <td>${esc(k.vendor)} (${esc(k.product)})</td>
      <td>${esc(k.name || 'Critical Flaw')}</td>
  ...
  const rwRows = briefing.ransomwareActivity.map((r) => `
    <tr>
      <td style="font-weight:700;color:#7c3aed">${esc(r.groupName)}</td>
      <td>${esc(r.victimName)}</td>
      <td>${esc(r.sector || 'Enterprise')}</td>
      <td>${esc(r.country || 'Global')}</td>
    </tr>
  `).join('');
  ...
  const actionList = briefing.strategicActionItems.map((a) => {
    const idx = a.indexOf(':');
    if (idx > 0) {
      return `<li style="margin-bottom:8px"><strong>${esc(a.slice(0, idx))}:</strong> ${esc(a.slice(idx + 1).trim())}</li>`;
    }
...
<head>
  <meta charset="UTF-8">
  <title>${esc(briefing.title)}</title>
  <style>
...
<body>
  <div class="report-card">
    <h1>🏛️ ${esc(briefing.title)}</h1>
    <div class="meta">
      <strong>Date:</strong> ${esc(briefing.date)} &nbsp;|&nbsp; <strong>Classification:</strong> TLP:AMBER &nbsp;|&nbsp; <strong>Generated by:</strong> News Feeder Bot SOC Intelligence
    </div>

    <div class="summary-box">
      <strong>Executive Overview:</strong> ${esc(briefing.executiveSummary)}
    </div>
